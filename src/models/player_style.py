...
class PlayerStyleEmbedding(nn.Embedding):
    def __init__(self, elo_embeddings: nn.Embedding, n_players: int):
        ...
        super().__init__(num_embeddings=total_embeddings,
                         embedding_dim=elo_embeddings.embedding_dim)
        ...
        with torch.no_grad():
            best_weights = self.elo_embeddings.weight[self.max_maia_idx].detach(
            ).clone()
            self.players_embeddings.weight.data = best_weights.repeat(
                n_players, 1)

    def forward(self, indices: torch.Tensor):
        is_player = indices > self.max_maia_idx
        out = torch.zeros(*indices.shape, self.dim, device=indices.device)
