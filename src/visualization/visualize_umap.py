...
def visualize_umap(player_list, state_mode=False):
    ...
    centroids_path = f"{RESULT_FOLDER}/centroids_map.pdf" if not state_mode else f"{RESULT_FOLDER}/centroids_map_state.pdf"
    style_comparison_path = f"{RESULT_FOLDER}/style_comparison.pdf" if not state_mode else f"{RESULT_FOLDER}/style_comparison_state.pdf"
